...
void fsi_write_and_advance()
{
    ...
    int subcycling = !precicec_isCouplingOngoing();
    double timestep_limit = 0.0;
...
void set_mesh_positions(Domain* domain)
{
    ...
    // Edited
    precicec_setMeshVertices(mesh_name, wet_nodes_size, initial_coords, displ_indices);
    precicec_setMeshVertices(mesh_name, wet_face_size, face_coords, face_indices);
...
void read_displacements(Dynamic_Thread* dt)
{
    ...
    if (dynamic_thread_node_size[thread_index] > 0) {
        ...
        precicec_readData("moving_base_nodes", "Displacements", dynamic_thread_node_size[thread_index], displ_indices, 0.0, displacements);
